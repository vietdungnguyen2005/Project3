...
- `components/ledger-grid.tsx` uses TanStack Table for the row model and `useVirtualizer` from `@tanstack/react-virtual` for the render window.
...
- `npm run proof` confirms 100,000 records and source-level virtualization.
...
- `next.config.ts` disables `X-Powered-By`, sets CSP, frame denial, referrer policy, permissions policy, and removes console calls in production.
...
- E2E tests confirm browser requests to `/api/ledger` do not carry private credentials and API responses are no-store/nosniff.
